...
```python
import sys
# Thay đổi đường dẫn dưới đây thành thư mục chứa "Animeow_Enjo_Pipeline" trên máy của bạn
path = r"e:/AI_Work/Blender/Maya_script"
if path not in sys.path:
    sys.path.insert(0, path)

import Animeow_Enjo_Pipeline
import Animeow_Enjo_Pipeline.menu as menu
menu.create_menu()
Animeow_Enjo_Pipeline.show()
```
