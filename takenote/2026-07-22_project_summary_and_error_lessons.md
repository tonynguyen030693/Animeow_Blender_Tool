...
- **Dự án**: Animeow Toolkit (Blender) & Animeow Maya Toolboard / Enjo Pipeline (Maya)
...
## 🚀 II. TỔNG HỢP TIẾN TRÌNH & TÍNH NĂNG THEO NGÀY THÁNG

### 🎨 1. Phân Hệ Blender Addon (`animeow_toolkit`)
- **[01/07/2026] Snap Similar Parts (`snap_aligner.py`)**: Tự động ghép nối các mảnh mô hình bị rã về đúng vị trí gốc dựa trên Mesh Data, Mesh Name, và Geometry (Local Bounding Box + số đỉnh/cạnh/mặt). Hỗ trợ Absolute Snap và Relative Snap (tìm mốc Z thấp nhất).
- **[02/07/2026] Anim Linker & Cặp Locator Kép**: Sử dụng cặp Empty lồng nhau `loc_parent_` (Hook) và `loc_child_` (Offset sạch key), tự động chuyển ma trận thế giới (`loc_temp`), nướng animation và hỗ trợ Space Switch giữ nguyên vị trí trực quan.
- **[02/07/2026] Kiến trúc OOP & UI Tab Ngang (Tab Editor Layout)**: Đóng gói toàn bộ các addon rời trước đây thành 1 bộ **Animeow Toolkit** duy nhất. Thanh Tab Ngang N-Panel: `[ 🎯 Linker ] [ 📋 Copy ] [ 🔢 Rounder ] [ 🎬 Baker ] [ 🦴 Picker ]`.
- **[03/07/2026] Smart Baker & Thuật toán 2-Pass Bảo vệ Cực trị**: Bake nội bộ ở Step=1 -> **Pass 1** giữ 100% các dáng cực trị (Local Extremes) -> **Pass 2** lọc keyframe tuyến tính theo threshold. Tự động đổi nội suy về `Bezier` và tay cầm `AUTO`. Tương thích chuẩn **Slotted Actions (Blender 5.0+)**.
- **[03/07/2026] Global Animation Layers**: Quản lý layer không phá hủy trên NLA. Xử lý triệt để lỗi NLA rỗng, lỗi nhầm Base track. Gộp Layer an toàn bằng cách duy trì Pose Mode khi Bake để nhân vật không bị sập về T-pose.
- **[03/07/2026] Anim Copy (Paste Mirror)**: Bổ sung 6 tùy chọn lật đối xứng độc lập (TX, TY, TZ, RX, RY, RZ).

### 🎬 2. Phân Hệ Maya Pipeline & Toolboard (`Maya_script` & `Maya_script_2020_python_2`)
- **[08/07/2026] Xuất Anim Tự Động khi Publish File**: Tự động gọi Studio Library API (`mutils.saveAnim`) xuất `all_assets.anim` và file anim lẻ từng Namespace khi Publish File.
- **[08/07/2026] Khởi Tạo Maya Toolboard Python 3 (Maya 2022+)**: Viết mới gói `animeow_maya_toolboard_v03` chạy trên Python 3.
- **[08/07/2026] Multi-Camera Playblast Exporter**: Cho phép chọn nhiều Camera quay song song có thanh tiến trình `QProgressDialog`.
- **[08/07/2026] Arc Tracker (NURBS Motion Trail)**: Trích xuất Rotate Pivot thế giới khi đóng băng viewport (`cmds.refresh(suspend=True)`), vẽ bằng đường cong NURBS mượt 100%, hiển thị key ticks đỏ (2x) & ticks vàng.
- **[14/07/2026] Smart Link (Double Locator System)**: Cặp locator `Anm_loc_link_parent_` & `Anm_loc_link_child_` trong group `Animeow_locator`. Hỗ trợ **Link to World**. Bake & Clean siêu tốc x10-x50 bằng core C++.
- **[14/07/2026] Curve Utilities & Favorites Window**: Tích hợp Smooth Keys Slider thời gian thực, Clean Neighborhood, Clean Sub-frame Keys (độ nhạy `1e-9`), Local Scale (MEL). Cửa sổ độc lập Favorites Standalone.
- **[14/07/2026] Overlapper (Python Porting)**: Chuyển đổi công cụ trễ chuyển động từ MEL sang Python 3.
- **[15/07/2026] ANM Hider & Temp Pivot**: ANM Hider tự tạo và lưu thuộc tính ẩn hiện các bộ phận của Rig. Temp Pivot tính trung điểm trọng tâm nhóm vật thể.
- **[16/07/2026] Selection Sets Manager**: Quản lý bộ chọn theo cây phân cấp (`QTreeWidget`), tự động gom nhóm dưới `Animeow_sets`, bắt buộc tiền tố `ANM_`, đệ quy ẩn/hiện thành viên, xuất/nhập file JSON kèm Smart Namespace Mapping.
- **[16/07/2026] Playblast Render Settings Mode**: Tự động lấy độ phân giải từ Render Settings (`defaultResolution`).
- **[21/07/2026] Smart World Bake & Fake Constraint**: Smart World Bake độc lập hỗ trợ nướng không gian thế giới bảo toàn cực trị hoặc nướng ngược về vật thể gốc, tùy chọn Không Constraint (`no_constraint`). Fake Constraint tính toán ma trận ma sát tương đối set key trực tiếp không làm rác Scene, hỗ trợ Multi-Follower.
- **[21/07/2026] Nâng Cấp Đồng Bộ Python 2 (Maya 2020)**: Hoàn thành porting gói `animeow_maya_toolboard_py2_v03` tương thích hoàn hảo Maya 2020 Python 2.7.
- **[22/07/2026] Gộp & Quy Hoạch Thư Viện Studio Library (`Z:\Animeow_Production\Enjo_Library`)**:
  - Gop 2 thư viện Kidsong & Lolo về 1 thư viện chuẩn chung.
  ...
  - UI Tab 2 hàng nút mở nhanh danh mục: `👤 Characters`, `🐾 Animals`, `🚗 Props & Vehicles`, `✋ Common Poses`, `📖 Studio Library Tổng`.
- **[22/07/2026] Chuẩn Hóa Tên File & Shot Version Auto-Cleaner**: Tự động gọt tiền tố/hậu tố trùng (`_Anim_Anim`), tự động tăng Version file chuẩn (`_v01`, `_v02`, `_v03`...), xóa file Maya bẩn trực tiếp trên UI.
...
1. **[22/07/2026] Bẫy "Lỗi Ảo" Traceback (Indirect Error Reporting)**:
...
2. **[22/07/2026] Xung đột Cú pháp Relative Import (Dấu chấm `.`)**:
...
3. **[22/07/2026] Bẫy Cache RAM & Bytecode `.pyc` của Python 2**:
...
4. **[15/07/2026] Lỗi Mã hóa Ký tự Unicode & Tiếng Việt (Python 2.7)**:
...
5. **[15/07/2026] Lỗi Tràn Ngăn Xếp Đệ Quy (`maximum recursion depth exceeded`)**:
...
6. **[15/07/2026] Lỗi `'NoneType' object is not callable` khi Reload Module**:
...
1. **[03/07/2026] Lỗi Slotted Actions (Blender 5.0+)**:
...
2. **[03/07/2026] Lỗi Mất Animation khi Gộp Layer (NLA Bake Context Bug)**:
...
3. **[03/07/2026] Lỗi NLA Track Rỗng khi Xóa Layer**:
...
4. **[02/07/2026] Lỗi UILayout.label Keyword Argument Error (Blender 5.1 API)**:
...
5. **[02/07/2026] Lỗi Tích tụ Locator Rác (`.001`, `.002`, `.003`)**:
...
1. **[22/07/2026] Lỗi Lặp Tên File & Trùng Shot (`_Anim_Anim_v01_1.ma`)**:
...
2. **[22/07/2026] Thư mục Studio Library Lộn xộn & Đặt tên Sai quy chuẩn**:
...
3. **[15/07/2026] Lỗi Quay Playblast Video Siêu Nhỏ (1% Scale) & Mất Tiếng**:
...
4. **[21/07/2026] Lỗi Đổi tên Locator Tự động làm Hỏng Select (World Bake)**:
