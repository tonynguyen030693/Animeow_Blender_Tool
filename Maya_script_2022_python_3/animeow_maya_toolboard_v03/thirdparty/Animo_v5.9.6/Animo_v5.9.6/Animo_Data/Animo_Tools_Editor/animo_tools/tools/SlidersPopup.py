...
def get_scale_factor():
    ...
    try:
        ...
        # Base size multiplier (1.08 = 8% larger base)
...
def scale_size(size):
    """Scale a size value based on DPI - alias for dpi()"""
    return dpi(size)
...
class CustomSlider(QtWidgets.QSlider):
    ...
    def paintEvent(self, event):
        ...
        font = painter.font()
        font.setPointSize(7)  # Don't scale pt - it's already DPI-aware
        font.setBold(True)
...
class SimplifiedTweenUI(QtWidgets.QDialog):
    ...
    def __init__(self, parent=get_maya_main_window()):
            ...
            self.setWindowOpacity(0.69)  # Static glassy opacity
            ...
            # Position will be set by toggle function at cursor
            self.old_pos = None
            ...
            # macOS-specific: Start a background timer that continuously checks 
            # mouse position for auto-close, since leaveEvent can be unreliable
            if IS_MACOS:
                self._background_check_timer = QtCore.QTimer()
                self._background_check_timer.timeout.connect(self._background_mouse_check)
                self._background_check_timer.start(200)  # Check every 200ms
    ...
    def closeEvent(self, event):
        # Stop all timers to prevent callbacks after close
    ...
    def _background_mouse_check(self):
        """Background check for mouse position on macOS.
        This provides a fallback for when leaveEvent doesn't fire properly."""
        # Don't close if any slider is being dragged
        ...
        if not expanded_rect.contains(cursor_pos):
            ...
            # Close after mouse has been outside for ~600ms (3 checks)
            if self._outside_count >= 3:
                if hasattr(self, '_background_check_timer'):
                    self._background_check_timer.stop()
                self.close()
        else:
            # Reset counter when mouse is inside
            self._outside_count = 0

    def force_deactivate_window(self):
        """Check if mouse is outside tolerance zone and close if so.
        This is called after slider interactions to ensure window closes on macOS."""
        # Don't close if any slider is being dragged
        if (self.tween_mouse_pressed or self.blend_mouse_pressed or 
            self.scale_mouse_pressed or self.cascade_mouse_pressed):
            return
        ...
        # Add tolerance zone - larger on macOS for better reliability
        tolerance = scale_size(40) if IS_MACOS else scale_size(27)
        
        # Expand the widget rect by tolerance
        expanded_rect = widget_rect.adjusted(-tolerance, -tolerance, tolerance, tolerance)
        
        # Close if cursor is outside the tolerance zone
        if not expanded_rect.contains(cursor_pos):
            self.close()
    ...
    def leaveEvent(self, event):
        """Mouse leaves UI - start close timer (with tolerance zone)"""
        # Don't start close timer if any slider is being dragged
        if (self.tween_mouse_pressed or self.blend_mouse_pressed or 
            self.scale_mouse_pressed or self.cascade_mouse_pressed):
            QtWidgets.QDialog.leaveEvent(self, event)
            return
            
        # Start a repeating timer to continuously check if mouse is far enough
        if not hasattr(self, '_close_timer'):
            self._close_timer = QtCore.QTimer()
            self._close_timer.timeout.connect(self._check_and_close)
        
        # Start repeating check - faster on macOS for better responsiveness
    ...
    def enterEvent(self, event):
        """Mouse enters UI - stop the close timer and reset background check"""
        if hasattr(self, '_close_timer') and self._close_timer.isActive():
            self._close_timer.stop()
        # Reset background check counter
        self._outside_count = 0
        QtWidgets.QDialog.enterEvent(self, event)
    
    def _check_and_close(self):
        """Check if mouse is still outside the UI with tolerance zone"""
        # Don't close if any slider is being dragged
        if (self.tween_mouse_pressed or self.blend_mouse_pressed or 
            self.scale_mouse_pressed or self.cascade_mouse_pressed):
            return
        ...
        # Add tolerance zone - larger on macOS for better reliability
        # About 2-3 cm depending on DPI
        tolerance = scale_size(40) if IS_MACOS else scale_size(27)
        
        # Expand the widget rect by tolerance
        expanded_rect = widget_rect.adjusted(-tolerance, -tolerance, tolerance, tolerance)
        
        # Only close if cursor is outside the tolerance zone
        if not expanded_rect.contains(cursor_pos):
            if hasattr(self, '_close_timer'):
                self._close_timer.stop()
            self.close()
        # If mouse is back in tolerance zone, keep checking
    ...
    def fast_linear_inbetween(self):
        ...
        # Close the UI after operation (non-persistent version)
        self.close()
...
def toggle_simplified_tween_ui():
    ...
    # Reset scale factor so it detects the screen where cursor is now
    reset_scale_factor()
