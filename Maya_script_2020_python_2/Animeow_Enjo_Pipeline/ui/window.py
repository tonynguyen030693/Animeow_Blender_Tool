...
class StudioLibraryManagerDialog(QtWidgets.QDialog):
    ...
    def on_format_num_selected(self):
        ...
        progress = QtWidgets.QProgressDialog(u"Đang quét và định dạng 3 chữ số cho Studio Library...", u"Hủy", 0, 100, self)
        progress.setWindowTitle(u"Tiến Trình Định Dạng Studio Library")
        ...
        def update_progress(current, total, current_item):
            if total > 0:
                val = int((float(current) / float(total)) * 100)
                progress.setValue(val)
                progress.setLabelText(u"Đang xử lý (%d/%d):\n%s" % (current, total, current_item))
            QtWidgets.QApplication.processEvents()

        try:
            ok, msg = self.main_ui.file_manager.format_studiolibrary_item_numbers(
                lib_dir=lib["path"],
            ...
            progress.close()
            QtWidgets.QMessageBox.critical(self, u"Lỗi", u"Lỗi thực thi: %s" % str(e))
