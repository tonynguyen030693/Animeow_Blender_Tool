...
def copy_xform_data():
    ...
    if EndRange - StartRange == 0:
        ...
        copy_animation_range(selection, StartRange, EndRange)
...
def copy_current_frame_transforms(selection):
    ...
    for obj in selection:
        try:
            ...
            animation_data[obj] = {
                str(current_frame): {
                    "matrix": [round(m, 6) for m in world_matrix]
                }
            }
            
        except Exception as e:
            cmds.warning("Failed to get matrix data for {0}: {1}".format(obj, str(e)))
    ...
    try:
        ...
        cmds.inViewMessage(
            amg='<span style="color:#4ca6e6;">Xform Copied</span>',
            pos='botCenter',
            fade=True
        )
