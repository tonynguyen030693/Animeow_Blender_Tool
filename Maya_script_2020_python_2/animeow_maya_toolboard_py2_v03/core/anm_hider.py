...
def get_hider_icon(icon_name):
    ...
    if icon_name.endswith("_Empty.png"):
        # Trang thai rong (chua co set): Trang mo nhe de de nhin nhung khong bi sang qua (alpha = 100)
        color = QtGui.QColor(255, 255, 255, 100)
    elif icon_name.endswith("_Off.png"):
        # Trang thai tat (dang an): Trang mo vua phai (alpha = 170)
        ...
        # Trang thai bat (dang hien): Cyan sang noi bat cua Animeow (#00BCD4)
        color = QtGui.QColor(0, 188, 212, 255)
    else:
        # Cac nut tien ich chuc nang khac: Trang sang ro rang (alpha = 230)
    ...
    if is_extra:
        # Tu dong ve icon so [1-7] bo goc cuc dep dong bo giao dien
    ...
    # Ap dung bo loc mau bang QPainter (CompositionMode_SourceIn) giu nguyen kenh alpha
...
def createSettingsHider():
    ...
    # 1. Tao group ANM_Hider_Settings neu chua co trong scene
    # Su dung bien day du ANM_Hider_Settings (da chua namespace)
    if cmds.objExists(ANM_Hider_Settings) == 0:
        cmds.group(em=True, n=ANM_Hider_Settings)
        # Lock cac thuoc tinh mac dinh cua Group
    ...
    # 2. Kiem tra va bo sung cac thuoc tinh trang thai (neu thieu)
    # Rat quan trong khi animator mo scene cu da co san node ANM_Hider_Settings nhung thieu Extra 4-7
    required_attrs = [
    ...
    for attr_name in required_attrs:
        ...
        if cmds.objExists(full_attr) == 0:
            try:
                ...
                print("Error adding attribute %s: %s" % (attr_name, e))
...
def get_namespace_from_object(obj):
    # Chi xet phan node name truoc dau cham component "." de tranh nham ky tu ":" trong chi so mat luoi (vi du: .f[34:50])
    node_part = obj.split(".")[0]
    # Lay phan ten co so (bo qua duong dan cha | neu co)
...
def get_state_attr_name(set_name_without_ns):
    # Quy doi ten Set sang ten Attribute tuong ung tren Settings node
    # Vi du: Head_ANM_Hider -> Head_Hider
...
def declaringNameSpaces():
    ...
    if len(currentSel) > 0:
        # Thu lay namespace qua referenceQuery truoc
        try:
            ns = cmds.referenceQuery(currentSel[0], namespace=True, shortName=True)
        except:
            # Neu la rig Import, trich xuat truc tiep tu ten vat the
...
def addSelectionToSet():
    ...
    try:
        ...
        if len(sel) > 0:
            # 1. Tu dong nhan dien Namespace tu vat the dang chon de dong bo
            ns = get_namespace_from_object(sel[0])
            target_ns_hider = ns + ":" if ns else ""
            
            # Neu namespace khac voi namespace hien hanh cua Hider, tien hanh cap nhat lai
            ...
            # Cap nhat lai bien setHider theo namespace moi
            setHider = namespaceHider + setHider.split(":")[-1]
            
            # 2. Neu set cua bo phan nay chua ton tai trong scene, tu dong khoi tao luon
            ...
            if cmds.objExists(setHider):
                ...
                # Cap nhat giao dien de sang cac icon len
                if _hider_window_instance:
                    _hider_window_instance.check_all_button_states()
            else:
                cmds.warning("Khong the tao hoac tim thay Set: %s" % setHider)
        else:
            cmds.warning(nothingSelected)
    except Exception as e:
        print("Error in addSelectionToSet: %s" % e)
        createSettingsHider()
...
def showOrHideButton():
    ...
    try:
        ...
        print("Error in showOrHideButton: %s" % e)
        createSettingsHider()
...
def ShowOrHideAllSets():
    ...
    # Chi giu lai cac con thuc su la objectSet (tranh cac cau truc mesh, component bi lot vao)
...
def saveSetsHider():
    if namespaceHiderForWindow == 'Created in the scene':
        ...
        if SourceFile:
            ...
            fileHandle.write("## Check all icons\nimport %s.core.anm_hider as fh; fh.checkAllIconSets()" % pkg_name)
            fileHandle.close()
...
def LoadSetsHider():
    try:
        ...
        cmds.error("%s: %s" % (errorLoadSets, e))
...
def SelectTemplateLineWindow():
    ...
    cmds.separator()
    cmds.button(l='1- Grow selection', c='import %s.core.anm_hider as fh; fh.growSelection()' % pkg_name)
    cmds.separator()
    cmds.button(l='2- Filter Only Curves', c='import %s.core.anm_hider as fh; fh.filterOnlyCurves()' % pkg_name)
    cmds.separator()
    cmds.button(l='3- Print number selected', c='import %s.core.anm_hider as fh; fh.printSelected()' % pkg_name)
...
class ANMHiderWindow(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.is_edit_mode = True  # Mac dinh mo rong de animator thay utilities
        self.faces_selection_mode = True  # Mac dinh chon Faces
        self.buttons = {}
        
        # Load namespaces & settings (Phai chay truoc setup_ui de khoi tao gia tri cho cac bien global nhu Head_Hider)
        declaringNameSpaces()
        createSettingsHider()
    ...
    def setup_ui(self):
        ...
        # Tag hien thi Namespace / Character dang chon
        ...
        # 2. Hang nut ngang (Bo phan co the & Set dieu khien)
        self.buttons_layout = QtWidgets.QHBoxLayout()
        self.buttons_layout.setSpacing(4)
        self.buttons_layout.setContentsMargins(2, 0, 2, 0)
        
        # Nut Edit Mode (Contract/Expand)
        ...
        # List dinh nghia cac nut bam Hider chinh
        self.hider_defs = [
        ...
        for key_name, var_name, label, tooltip, conn in self.hider_defs:
            ...
            # Dung lambda boc de tranh mat context
        ...
        # 3. Widget Utilities (Hang duoi, co the thu gon duoc)
        ...
        # Nut Poly / Curves Selection Mode
        ...
        # Set size ban dau cua window
        self.adjustSize()
        self.setFixedSize(self.width(), self.height())

    def apply_stylesheet(self):
        # CSS phang phong cach toi gian mau Cyan cao cap dong bo voi Animeow Toolboard v02
        self.setStyleSheet("""
            QDialog {
    ...
    def update_namespace_info(self):
        ...
        self.namespace_label.setText("Rig: %s" % namespaceHiderForWindow)

    def refresh_ui(self):
        # Cap nhat Edit Mode icon
        ...
        # Co dan kich thuoc dialog muot ma
        self.setMinimumSize(0, 0)
        self.setMaximumSize(16777215, 16777215)
        self.adjustSize()
        self.setFixedSize(self.width(), self.height())
        
        # Cap nhat icon cua tung nut bam theo trang thai set trong scene
        self.check_all_button_states()

    def check_all_button_states(self):
        # Duyet qua cac nut bo phan co the de doi icon
        for key_name, var_name, _, _, _ in self.hider_defs:
            if not cmds.objExists(var_name):
                # Neu set chua ton tai trong scene -> Empty
    ...
    def show_context_menu(self, pos, key_name, var_name):
        # Context Menu chuot phai bo goc Cyan phang cuc dep
    ...
    def on_about(self):
        QtWidgets.QMessageBox.information(
            self, "About", 
            "%s\n\nDuoc nang cap va thiet ke lai giao dien phang PySide2 toi gian cho bo cong cu Animeow Maya Toolboard.\n\nOriginal tool by Francisco Cerchiara Montero." % versionHider
        )
...
def show_hider():
    """Ham khoi chay chinh bang giao dien PySide2 cao cap"""
    global _hider_window_instance
    setup_icons_path()
    
    # Dong cua so cu neu co
