...
global proc OverlapperRelease()
{
	...
	string $window = `window  -title "Overlapper 1.2(beta)"
	       -toolbox 1
	       -s 1
	       -menuBar 1 
	       -widthHeight 172 133
...
proc Overlapper()
{
	...
	setAttr ($SelectedControls[$i-1]+"LastOrientJoint.translateX") $averageLenghtJoints;
	  ...
	  if ($useFirstCtrl==1&&$TRANSLATEmode==0)
	{
    ...
    if ($useFirstCtrl==1) {select -d $FirstSelectedCtrl;}
    $SelectedControls = `ls -sl`;
