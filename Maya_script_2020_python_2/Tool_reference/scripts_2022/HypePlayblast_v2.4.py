## Created by Ottoni Bastos and Gilberto Ribeiro for Hype.cg at 15/06/2016.
## Create Playblasts from scenes with Hype's standard attribute values.
##...
## v1.2 07/07/16 : Fix a bug where the split playblast would overwrite the .mov files on each other.
## v1.3 13/07/16 : Updated the standard name of the .mov file.
## v1.4 15/07/16 : Now the script works even when there are no shots in the sequence manager and fixed a bug with scene info HUD.
## v1.5 15/07/16 : Fixed a problem where the splitted playblasts would always use the same start frame and end frame.
## v1.6 18/07/16 : Fixed a problem where the script would crash if it couldn't find any sound node.Now it will build a soundless Playblast in this case.
## v1.7 28/07/16 : Add new option in the Status enumeration button called WIP.
## V1.8 17/08/16 : Fixed a sound list problem.
## V1.9 03/04/17 : Now the UI allows users to select the audio track to use on playblast.
## V2.0 07/04/17 : Fix some bugs.
## V2.1 12/05/17 : Fix a HUD visibility problem.
...
def doPlayBlastIWinUI():
    ...
    win = mc.window('doPlayBlastWin', t='Hype Playblast v2.4', s=False)
    ...
    # new ROW    
    # animator name
    mc.rowColumnLayout( parent='doPlayBlastMainLay', numberOfColumns=2, columnWidth=[(1, 150),(2, 168)] )
    mc.optionMenu('statusType',w = 130 ,label = 'Status:')
    mc.checkBox("use_track_sound_checkbox",label = "Use Trax sound") 
    options = ['Blocking','Blocking Plus','Polish','WIP']
    for opt in options:
    ...
    if Clist != None:
        ...
        if cameras == None:
            ...
            for c in cameras:
                ...
                mc.camera(c,e=True,dgm= False,dr=False,ovr=1.0)
...
def makePlayblast(startFrame,endFrame,fileName,soundName,wh,pathFile=mc.workspace(q=True,fn=True)+'/movies/animation/' ):
    ...
    elif mc.checkBox("use_track_sound_checkbox",q=True, value=True):
        mc.playblast(  format="qt", startTime= startFrame, endTime= endFrame, filename=pathFile+fileName,useTraxSounds = True,forceOverwrite=True, showOrnaments=True, offScreen=True,  fp=4, percent=100, compression="H.264", quality=100, widthHeight=wh) 
    else:
