...
class GRAPH_PT_toolboard_main(bpy.types.Panel):
    ...
    bl_label = "⚡ Animeow Graph Toolboard"
    ...
    def draw(self, context):
        ...
        # Display the custom Animeow logo if loaded
        pcoll = preview_collections.get("main")
        if pcoll and "animeow_logo" in pcoll:
            logo_icon = pcoll["animeow_logo"]
            row_logo = layout.row(align=True)
            row_logo.alignment = 'CENTER'
            row_logo.template_icon(icon_value=logo_icon.icon_id, scale=4.0)
            layout.separator()
...
def register():
    ...
    logo_path = os.path.join(addon_dir, "Animeow_logo.png")
    if os.path.exists(logo_path):
        pcoll.load("animeow_logo", logo_path, 'IMAGE')
