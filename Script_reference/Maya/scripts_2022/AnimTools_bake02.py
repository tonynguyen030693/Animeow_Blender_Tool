...
class FAnimImportExport():
    ...
    def importMocapAnim(self,*args):
        ...
        for jnt in outJntData:
            ...
            animJnt=importNS+':'+jnt
            if(cmds.objExists(animJnt) and cmds.objExists(characterNS+ctrl)):
                ...
                con=cmds.parentConstraint(animJnt,newCtrl,mo=1)
                cmds.setAttr(con[0]+'.interpType',0)

                newObjMap[animJnt]=[newCtrl,characterNS+ctrl]
                if(pWorld):
                    pWorld=False
                else:
                    pJnt=cmds.listRelatives(animJnt,p=1)[0]
                    newPJnt=newObjMap[pJnt][0]
                    newPCtrl=newObjMap[pJnt][1]
                    cmds.parent(newCtrlGrp,newPCtrl)

        
        rootJnt=newObjMap.items()[0][0]
        ...
        for animJnt in newObjMap:
            newCtrl,chCtrl=newObjMap[animJnt]
            for attr in attrList:
                isKeyable=cmds.getAttr(chCtrl+'.'+attr,k=1)
                if(not isKeyable):
                    continue
        ...
        cmds.progressWindow(t='Doing Nothing',pr=0,max=keyNum,st='Sleeping: 0%',ii=True )
        for idx in range(keyNum):
idx=0
            if cmds.progressWindow(q=1,ic=1):
                break
            cmds.progressWindow( e=True,s=1, status='Sleeping: %s '%(idx/keyNum))
            curT=rootAnimCurve.time(idx).value()
            cmds.currentTime(curT)
            for getAttrStr in keyAnimCurveMap:
getAttrStr=keyAnimCurveMap.items()[0][0]
